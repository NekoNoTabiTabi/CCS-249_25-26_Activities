...
WIKI_URL = "https://en.wikipedia.org/wiki/Disco_Elysium"
RANDOM_SEED = 42
...
def visualize_pca(model: Word2Vec, words: List[str], filename: str = "word_vectors_pca.png"):
    """
    Visualize word vectors using PCA to reduce dimensions to 2D.
    Only includes words that exist in the model vocabulary.
    """
    # Filter words that exist in the model
    existing_words = [w for w in words if has_word(model, w)]
    
    if len(existing_words) < 2:
        print(f"Not enough words in vocabulary for PCA visualization (found {len(existing_words)})")
        return
    
    # Get vectors for the words
    vectors = np.array([model.wv[word] for word in existing_words])
    ...
    print(f"\nPCA visualization saved to: {filename}")
    print(f"Visualized {len(existing_words)} words (from {len(words)} requested)")
...
def main():
    ...
    probe_words = [
        "harry", "harrier", "du", "bois", "kim", "kitsuragi",
        "revachol", "martinaise", "insulinde", "pale",
        "thought", "cabinet", "skill", "checks", "dialogue",
        "detective", "amnesia", "murder", "hanged", "investigation",
        "alcohol", "drug", "communist", "revolution"
    ]

    print_top_neighbors(model, probe_words, topn=8)

    # PCA Visualization with at least 20 known words
    pca_words = [
        "harry", "du", "bois", "kim", "kitsuragi",
        "revachol", "martinaise", "insulinde", "pale", "city", "district",
        "thought", "cabinet", "skill", "checks", "dialogue", "points",
        "detective", "amnesia", "murder", "hanged", "investigation",
        "alcohol", "drug", "communist", "revolution", "communism",
        "political", "police", "partner", "case", "body"
    ]
    
    visualize_pca(model, pca_words, filename="word_vectors_pca.png")
    ...
    relatedness_test = [
        ...
        ("harry", "kim", 0.95),           
        ("harry", "detective", 0.90),     
        ("kim", "detective", 0.85),       
        ("harry", "amnesia", 0.85),       
        ("harry", "alcohol", 0.80),       
        ("revachol", "city", 0.90),       
        ("revachol", "martinaise", 0.85), 
        ("insulinde", "isola", 0.85),     
        ("thought", "cabinet", 0.95),     
        ("skill", "checks", 0.90),        
        ("dialogue", "trees", 0.85),     
        ("detective", "investigation", 0.90),      
        ("detective", "crime", 0.80),
        ("alcohol", "amnesia", 0.80),      
        ("harry", "kitchen", 0.05),
        ("revachol", "tractor", 0.02),
        ("kim", "movie", 0.25),
    ]
    ...
    analogy_test = [
        ("harry", "detective", "kim", "partner"),      
        ("harry", "du", "kim", "kitsuragi"),           
        ("revachol", "city", "martinaise", "district"), 
        ("thought", "cabinet", "skill", "check"),       
    ]
