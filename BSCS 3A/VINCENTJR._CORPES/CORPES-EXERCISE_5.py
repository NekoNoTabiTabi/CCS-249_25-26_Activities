...
WIKI_URL = "https://en.wikipedia.org/wiki/Chimango_caracara"
RANDOM_SEED = 42
...
def plot_pca_vectors(model: Word2Vec, words: List[str], save_path: str = "pca_word_vectors.png"):
    ...
    plt.scatter(reduced[:, 0], reduced[:, 1], s=60, alpha=0.85, color="#1f77b4")

    for i, word in enumerate(known_words):
        plt.annotate(word, (reduced[i, 0], reduced[i, 1]), fontsize=9, alpha=0.9)
    ...
    print(f"\nPCA words used: {len(known_words)}/{len(words)}")
    missing_words = [w for w in words if w not in known_words]
    if missing_words:
        print("OOV words skipped in PCA:", ", ".join(missing_words))
...
def main():
    ...
    probe_words = [
        "chimango", "caracara", "raptor", "falconidae", "tiuque",
        "scavenger", "predator", "habitat", "argentina", "plumage"
    ]
    ...
    # Domain-specific relatedness test set for Chimango Caracara
    relatedness_test = [
        ("chimango", "caracara", 0.98),  
        ("chimango", "tiuque", 0.90),      
        ("caracara", "raptor", 0.85),     
        ("falconidae", "falcon", 0.90),   
        ("scavenger", "carrion", 0.85),    
        ("predator", "prey", 0.75),       
        ("argentina", "chile", 0.80),      
        ("nest", "eggs", 0.85),           
        ("plumage", "feathers", 0.95),    
        ("habitat", "savanna", 0.70),     
        ("bird", "tractor", 0.05),        
        ("caracara", "kitchen", 0.02),     
    ]
    ...
    analogy_test = [
        ("predator", "prey", "scavenger", "carrion"),
        ("argentina", "pampas", "chile", "andes"),
        ("male", "yellow", "female", "pink"), # Referring to skin color around eyes
    ]
   ...
   # Example direct similarity checks
    ...
    # These pairs test how the model handles synonyms, habitats, and unrelated concepts
    check_pairs = [
        ("chimango", "caracara"),   
        ("chimango", "tiuque"),     
        ("raptor", "predator"),     
    ...
    # PCA visualization with at least 20 known vocabulary items
    pca_words = [
        "chimango", "caracara", "milvago", "polyborus", "phalcoboenus",
        "tiuque", "falconidae", "caracaras", "bird", "family",
        "predator", "scavenger", "omnivorous", "prey", "argentina",
        "chile", "uruguay", "southern", "journal", "research",
        "roadside", "surveys", "description", "image", "wikispecies"
    ]
    plot_pca_vectors(model, pca_words, save_path="pca_word_vectors.png")
