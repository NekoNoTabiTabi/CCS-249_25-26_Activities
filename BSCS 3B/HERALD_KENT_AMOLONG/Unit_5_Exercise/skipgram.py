...
def fetch_wikipedia_article(url: str) -> str:
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; SGNS-Minecraft-Training/1.0)"
    }
...
def plot_pca_embeddings(
    ...
    out_path: str = "exercise_5_pca_embeddings.png",
    max_words: int = 20,
):
    unique_words = []
    for w in candidate_words:
        if w not in unique_words and has_word(model, w):
            unique_words.append(w)

    selected_words = unique_words[:max_words]
    if len(selected_words) < 2:
        print("\nPCA skipped: not enough in-vocabulary words.")
        return
    ...
    plt.scatter(reduced[:, 0], reduced[:, 1], s=55, alpha=0.85)

    for i, w in enumerate(selected_words):
        plt.annotate(w, (reduced[i, 0], reduced[i, 1]), fontsize=9, alpha=0.9)

    var1 = pca.explained_variance_ratio_[0] * 100
    var2 = pca.explained_variance_ratio_[1] * 100
    plt.title(f"Minecraft Word Embeddings PCA (n={len(selected_words)})")
    ...
    print(f"\nPCA plot saved to: {out_path}")
    print(f"Words plotted ({len(selected_words)}): {', '.join(selected_words)}")
...
def main():
    ...
    probe_words = [
        "minecraft", "block", "game", "player", "world",
        "craft", "mine", "zombie", "creeper", "diamond"
    ]
    ...
    relatedness_test = [
        ...
        ("minecraft", "block", 0.95),
        ("minecraft", "game", 0.90),
        ("minecraft", "world", 0.85),
        ("zombie", "creeper", 0.90),
        ("diamond", "ore", 0.90),
        ("player", "craft", 0.85),
        ("mine", "cave", 0.80),
        ("build", "construct", 0.85),
        ("minecraft", "kitchen", 0.05),
        ("block", "tractor", 0.02),
        ("diamond", "movie", 0.15),
        ("zombie", "tree", 0.25),
    ]
    ...
    analogy_test = [
        ("minecraft", "block", "game", "world"),
        ("player", "craft", "zombie", "attack"),
        ("mine", "ore", "build", "structure"),
        ("diamond", "precious", "iron", "common"),  # likely OOV or fail, included as test
    ]
    ...
    pca_candidate_words = [
        "minecraft", "game", "player", "world", "mode", "survival", "creative",
        "block", "blocks", "craft", "crafting", "build", "building", "mine",
        "mining", "zombie", "creeper", "enderman", "village", "villager",
        "server", "multiplayer", "singleplayer", "mojang", "java", "edition",
        "microsoft", "console", "mobile", "update"
    ]
    plot_pca_embeddings(model, pca_candidate_words, out_path="exercise_5_pca_embeddings.png", max_words=20)

    # Save model
    model.save("exercise_5_skipgram_sgns_5window.model")
    print("\nSaved model to: exercise_5_skipgram_sgns_10window.model")
