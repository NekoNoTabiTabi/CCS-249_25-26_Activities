...
WIKI_URL = "https://en.wikipedia.org/wiki/Car"
RANDOM_SEED = 42


def article_root_from_url(url: str) -> str:
    name = url.rstrip("/").split("/")[-1].lower()
    name = name.replace("_", " ")
    name = re.sub(r"[^a-z0-9\s-]", "", name)
    return name.split()[0] if name else "topic"
...
def main():
    ...
    root = article_root_from_url(WIKI_URL)
    probe_words = [
        root, "vehicle", "automobile", "engine", "wheel",
        "model", "drive", "road", "speed", "traffic"
    ]
    ...
    relatedness_test = [
        # These scores in the pairs are just examples; adjust as needed.
        (root, "vehicle", 0.95),
        (root, "automobile", 0.90),
        (root, "engine", 0.85),
        ("automobile", "vehicle", 0.90),
        ("car", "truck", 0.70),
        ("car", "bicycle", 0.20),
        ("car", "kitchen", 0.05),
        ("car", "tractor", 0.30),
    ...
    analogy_test = [
        ("car", "automobile", "truck", "vehicle"),
        ("engine", "motor", "wheel", "tire"),
        ("road", "highway", "city", "street"),
    ]
