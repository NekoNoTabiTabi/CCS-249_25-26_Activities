...
def visualize_pca(model: Word2Vec, words: List[str]):
    ...
    # Reduce 100 dimensions down to 2 using PCA
    pca = PCA(n_components=2)
    result = pca.fit_transform(vectors)

    plt.figure(figsize=(12, 8))
    plt.scatter(result[:, 0], result[:, 1], edgecolors='k', c='purple')
    ...
    plt.title("Taylor Swift Word2Vec Vector Visualization (PCA)")
    plt.grid(True)
    plt.show()
...
def main():
    ...
    probe_words = [
        "taylor", "swift", "singer", "album", "music",
        "tour", "country", "pop", "grammy", "songs"
    ]
    ...
    analogy_test = [
        ("country", "pop", "fearless", "1989"),
        ("singer", "song", "writer", "lyrics"),
        ("album", "songs", "tour", "concerts"),
    ]
...
# PCS Visualization words
    words_to_plot = [
        "taylor", "swift", "singer", "artist", "vocalist",
        "music", "country", "pop", "grammy", "award", "records",
        "album", "songs", "lyrics", "fearless", "reputation", 
        "folklore", "evermore", "red", "1989", "lover", "midnights",
        "tour", "eras", "concert", "performance"
    ]

    print("\nGenerating PCA Visualization...")
    visualize_pca(model, words_to_plot)
